...
const apiConfirmLogin = apiUrl.concat("token/validate/");//get parameter token needed
...
const apiUserCreatePurchaseOrder = apiUrl.concat("user/order/create");
const apiUserEditPurchaseOrder = apiUrl.concat("user/order/edit");
const apiUserInfoPurchaseOrder = apiUrl.concat("user/order/info");
const apiUserSentPurchaseOrder = apiUrl.concat("user/orders/sent")
const apiUserReceivedPurchaseOrder = apiUrl.concat("user/orders/received")
...
const apiFlagPost = apiUrl.concat("user/flag/create");
const apiFlagInfo = apiUrl.concat("user/flag/info");
const apiFlagEdit = apiUrl.concat("user/flag/edit");
const apiUserSentFlags = apiUrl.concat("user/flags");
...
const loginUserConfirm = () => {
	...
	if(token != null){
		...
		xhttp.open("GET", `${apiConfirmLogin}?token=${token}`, true);
...
async function sendPOSTCallwithToken(url = '', data = {}) {
		...
		xhttp.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
				...
				if(statusObj == 400){
					...
					localStorage.clear();
					console.log("Cannot validate token, somethime is very wrong" , dataObj);
					logoutUser();
				}			
			}
		};
...
const getCurrentDateForJson = () => {
	...
	//let month = dateObj.getUTCMonth() + 1; // single digit
