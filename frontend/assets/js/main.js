...
const getUserSentOrdersCount = () => {	
	let id = getUserID();
	let token = getUserToken();
	let totalPurchaseOrdersSentCount = document.getElementById("totalPurchaseOrdersSentCount"); 
	...
	let successResponseReceivedOrders = (data) => {
		...
		if(statusObj == 200){
			...
			try{
				totalPurchaseOrdersSentCount.innerHTML = dataObjCount ;		
			}catch(err){
				// no need to do anything with the err
			}
	...
	let errorResponseSentOrders = (error) => {
		console.error("unable to set Sent orders count >>>> ",  error );
	}
		
	if(token != null){
		loading.style.display = "block";
		sendPOSTCallwithToken(apiUserSentPurchaseOrder, {"id" : id})
		.then(data => successResponseReceivedOrders(data))
		.catch(error => errorResponseSentOrders(error));		
	}
}

const getUserReceivedOrdersCount = () => {			

	let successResponseReceivedOrders = (data) => {
		let totalPurchaseOrdersReceivedCount = document.getElementById("totalPurchaseOrdersReceivedCount"); 
		let dataObjCount;
		let dataObjResults;
		...
		if(statusObj == 200){
			...
			try{
				totalPurchaseOrdersReceivedCount.innerHTML = dataObjCount ;		
			}catch(err){
				// no need to do anything with the err
			}
...
const getUserSentFlagsCount = () => {
	let successResponseFlags = (data) => {
		let totalFlagsReceivedCount = document.getElementById("totalFlagsReceivedCount"); 
		let dataObjCount;
		let dataObjResults;
		...
		if(statusObj == 200){
			...
			try{
				totalFlagsReceivedCount.innerHTML = dataObjCount ;		
			}catch(err){
				// no need to do anything with the err
			}
	...
	let errorResponseFlags = (error) => {
		console.error("unable to set flags/reports count >>>> ",  error );
	}
	...
	if(token != null){
		loading.style.display = "block";
		sendPOSTCallwithToken(apiUserSentFlags, {"user_id" : id})
		.then(data => successResponseFlags(data))
		.catch(error => errorResponseFlags(error));		
	}
}
...
window.onload = function(){	
	...
	try{
		...
		setTimeout(getUserSentOrdersCount ,1000 ); // set the count for user's purchase orders (sent) in the secondary menu ,did this to ensure all is loaded before display opt		
		setTimeout(getUserReceivedOrdersCount ,1000 ); // set the count for user's purchase orders (received) in the secondary menu ,did this to ensure all is loaded before display opt		
		setTimeout(getUserSentFlagsCount ,1000 ); // set the count for user's flags/reports in the secondary menu ,did this to ensure all is loaded before display opt		
	}catch(err){
